...
class VirtualBody:
    ...
    def __init__(self, world: SharedWorld):
        ...
        self.speed = 0.0          
        self.reward  = 0.0
        self.energy = 1.0         # 1.0 = Máxima energía
    ...
    def step(self, dt, motor_outputs: dict):
        """
        motor_outputs: Diccionario con llaves:
        - 'left', 'right': Motores principales
        - 'signals': Lista de 8 canales [0,1]
        - 'metabolism': Control de eficiencia energética
        """
        self.reward = 0
        ...
        # 2. Modelo Metabólico (Gasto de Energía)
        # El movimiento y el tamaño del cerebro consumen energía
        cost = (linear * 0.001 + abs(angular) * 0.005) * dt
        self.energy -= cost / self.metabolism_eff
        self.energy = np.clip(self.energy, 0, 1)

        # 3. Actualizar Señales de Comunicación
        ...
        # 5. Interacción con Comida (Recarga Energía)
        collision_result = self.world.check_food_collision(self.x, self.y)
        if collision_result:
            self.dist_to_food_prev = np.inf
            if collision_result == "JACKPOT": 
                self.reward = 15.0
                self.energy = 1.0 # Recarga total
            elif collision_result == "SOCIAL": 
                self.reward = 5.0
                self.energy = min(1.0, self.energy + 0.8)
            else: 
                self.reward = 1.0
    ...
    def get_sensory_input(self):
        """Retina Multi-Orgánica: 12 Canales (Clásicos + Comunicación)."""
        ...
        rel_angle = (angle_to - self.angle + np.pi) % (2 * np.pi) - np.pi
        
        # Ojos Clásicos (Canal 1-5)
        s1 = 1.0 / (1.0 + dist / 100.0)
        s2 = rel_angle / np.pi
        other = self.world.bodies[1] if self.world.bodies[0] == self else self.world.bodies[0]
        s3 = 1.0 / (1.0 + np.hypot(self.x - other.x, self.y - other.y) / 100.0)
        s4 = 1.0 / (1.0 + np.hypot(self.x - self.world.tool_x, self.y - self.world.tool_y) / 100.0)
        s5 = signature
        
        # Oído Social (Canal 6-9: Escuchando al otro)
        # Solo mapeamos los primeros 4 canales de la voz del otro por ahora
        s6_9 = other.signal_output[:4]
        
        # Estado Interno (Canal 10)
        s10 = self.energy
        
        return np.concatenate([[s1, s2, s3, s4, s5], s6_9, [s10]])

    def get_sensory_input(self):
        """Retina Multi-Orgánica (10 Canales): Combina visión, oído social y estado interno."""
    ...
    def step(self, dt, motor_outputs: dict):
        ...
        # Gasto Energético (Moverse cuesta, pensar también)
        # La eficiencia metabólica reduce este coste
        self.metabolism_eff = 1.0 + motor_outputs.get('metabolism', 0) * 2.0
        cost = (linear * 0.002 + abs(angular) * 0.008) * dt
        self.energy -= cost / self.metabolism_eff
        self.energy = np.clip(self.energy, 0, 1)
        ...
        if res:
            self.dist_to_food_prev = np.inf
            if res == "JACKPOT": self.reward = 15.0; self.energy = 1.0
            elif res == "SOCIAL": self.reward = 5.0; self.energy = min(1.0, self.energy + 0.8)
            else: self.reward = 1.0; self.energy = min(1.0, self.energy + 0.4)
            return True
        return False
...
def _new_step(self):
    ...
    if self.brain_state == BrainState.AWAKE:
        retina = self.body.get_sensory_input() # 10 Canales
    ...
    motor_outputs['left'] += np.random.uniform(0, 0.05)
    motor_outputs['right'] += np.random.uniform(0, 0.05)
    ...
    try:
        while True:
            ...
            def get_stat(sim):
                ...
                return f"N:{n} E:{sim.body.energy:.2f} {get_aura(sim.body.signal_output)}"
