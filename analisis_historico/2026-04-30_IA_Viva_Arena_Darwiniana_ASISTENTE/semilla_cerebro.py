...
# --- Interoception computacional: muestreo de recursos del sistema -----------
try:
...
_SYS_CACHE = {'ram': 0.0, 'cpu': 0.0, 'last_sample': -1.0}

def _sample_system_resources(time_ms, sample_every_ms=200.0):
    """Muestrea RAM/CPU del proceso actual; usa caché para evitar overhead."""
    if not _HAS_PSUTIL:
        return 0.0, 0.0
    if time_ms - _SYS_CACHE['last_sample'] >= sample_every_ms:
        try:
            _SYS_CACHE['ram'] = _psutil.virtual_memory().percent / 100.0
            _SYS_CACHE['cpu'] = _psutil.cpu_percent(interval=None) / 100.0
            _SYS_CACHE['last_sample'] = time_ms
        except Exception:
            pass
    return _SYS_CACHE['ram'], _SYS_CACHE['cpu']
...
class IntelligentCell:
    ...
    def update_biology(self, dt, neighbor_delta, ext_signal, neurotrophic, morphogen, brain_state):
        ...
        # En la v17.0 no hay energía metabólica. El foco es la integridad de la señal.
        ...
        if self.type == CellType.STEM:
            ...
            # v17.0: No hay muerte por baja energía
            pass
    ...
    def update_neural(self, dt, time, input_current, brain_state, neuromod=None):
        ...
        # v17.0: La excitabilidad ya no depende de la energía (Neurovascular perfecto)
        ...
        if self.v >= effective_thresh:
            ...
            for syn in self.synapses_out:
                syn.pre_spike(time)
            # v17.0: Disparar es metabólicamente gratis
            pass
    ...
    def update_glia(self, dt, neurons, synapses, local_energy=1.0):
        if self.type != CellType.GLIA:
            return
        # Helper persistente: evita crear/destruir un objeto por step y permite
        # que estado interno (p.ej. phagocytosis_count, ca_level) se acumule.
        helper = getattr(self, '_glia_helper', None)
        if self.subtype == GliaSubtype.ASTROCYTE:
            if not isinstance(helper, Astrocyte):
                helper = Astrocyte(self.id, self.pos)
                helper.ca_level = getattr(self, 'ca_level', 0.0)
                helper.vasodilation_signal = getattr(self, 'vasodilation_signal', 0.0)
                self._glia_helper = helper
            helper.pos = self.pos
            helper.update(dt, neurons, synapses, local_energy)
            # Espejos para compatibilidad con código que lee desde la célula
            self.ca_level = helper.ca_level
            self.d_serine = helper.d_serine
            self.vasodilation_signal = helper.vasodilation_signal
        elif self.subtype == GliaSubtype.OLIGODENDROCYTE:
            if not isinstance(helper, Oligodendrocyte):
                helper = Oligodendrocyte(self.id, self.pos)
                helper.maturation = getattr(self, 'maturation', 0.0)
                self._glia_helper = helper
            helper.pos = self.pos
            helper.update(dt, neurons)
            self.maturation = helper.maturation
            self.radius = helper.radius
        elif self.subtype == GliaSubtype.MICROGLIA:
            if not isinstance(helper, Microglia):
                helper = Microglia(self.id, self.pos)
                self._glia_helper = helper
            helper.pos = self.pos
            helper.update(dt)
            self.pos = helper.pos.copy()
    ...
    def can_divide(self, dopamine=0.5):
        ...
        return (self.type == CellType.STEM and self.state == CellState.PROLIFERATIVE
                ...
                and np.random.random() < merit_factor * 0.1)
...
class BrainSimulation:
        ...
        for pre in neurons:
            ...
            for post in neurons:
                ...
                if dist < max_dist and np.random.rand() < p_connect:
                    # Calcular retraso por mielinizaciÃ³n: tomar el oligo más maduro
                    # que cubra el axón (antes el último iterado ganaba arbitrariamente).
                    myelin_factor = 1.0
                    for oligo in glia_oligo:
                        ...
                        if d_oligo < oligo.radius and hasattr(oligo, 'maturation'):
                            myelin_factor = max(myelin_factor, 1.0 + 9.0 * oligo.maturation)
    ...
    def step(self):
        ...
        # v17.0: La salud glial ya no depende de la energía (Homeostasis perfecta)
        avg_energy = 1.0 
        ...
        if self.step_count % 500 == 0 and microglia:
            for mg in microglia:
                micro = getattr(mg, '_glia_helper', None)
                if not isinstance(micro, Microglia):
                    micro = Microglia(mg.id, mg.pos)
                    mg._glia_helper = micro
                micro.pos = mg.pos
        ...
        for cell in neurons:
            input_current = 0.0
            # En vigilia, oscilador interno suave (reducido de 2.0 a 0.3)
            # para que la entrada sensorial fisica del cuerpo domine la percepcion.
            if self.brain_state == BrainState.AWAKE:
                input_current += 0.3 * target
        ...
        # Eliminar apoptóticas y limpiar sinapsis huérfanas que apuntan a IDs muertos.
        # Sin esta limpieza, las post-sinápticas siguen iterando "fantasmas" en
        # update_conductances/get_current y _incremental_structural_plasticity
        # cuenta sinapsis muertas como reales (oculta el pánico real de la red).
        dead_ids = {c.id for c in self.cells if c.type == CellType.APOPTOTIC}
        self.cells = [c for c in self.cells if c.type != CellType.APOPTOTIC]
        if dead_ids:
            for c in self.cells:
                if c.type == CellType.NEURON:
                    if c.synapses_in:
                        c.synapses_in = [s for s in c.synapses_in if s.pre_id not in dead_ids]
                    if c.synapses_out:
                        c.synapses_out = [s for s in c.synapses_out if s.post_id not in dead_ids]
            # Limpiar utilidad neuronal: la selección darwiniana debe operar
            # sobre la red viva, no sobre IDs de neuronas que ya no existen.
            utility = getattr(self, '_neuron_utility', None)
            if isinstance(utility, dict):
                for did in dead_ids:
                    utility.pop(did, None)
        if not self.cells:
...
class SharedWorld:
    ...
    def __init__(self):
        # Comida Solo (Fácil)
        self.food_solo_x = np.random.uniform(50, 350)
        self.food_solo_y = np.random.uniform(50, 350)
        
        # Comida Social (Difícil, requiere dos)
        self.food_social_x = np.random.uniform(50, 350)
        self.food_social_y = np.random.uniform(50, 350)
        ...
        self.step_count = 0
        self.bodies = [] # Hasta 4 agentes
        self.era = 2 

    def update_world(self):
        self.step_count += 1

    def save_world(self, path="mundo_arena.pkl"):
        try:
            with open(path, 'wb') as f:
                data = {
                    'step_count': self.step_count,
                    'food_solo': (self.food_solo_x, self.food_solo_y),
                    'food_social': (self.food_social_x, self.food_social_y),
                    'tool': (self.tool_x, self.tool_y)
                }
                pickle.dump(data, f)
        except Exception:
            pass

    def load_world(self, path="mundo_arena.pkl"):
        try:
            if os.path.exists(path):
                with open(path, 'rb') as f:
                    data = pickle.load(f)
                    self.step_count = data['step_count']
                    self.food_solo_x, self.food_solo_y = data['food_solo']
                    self.food_social_x, self.food_social_y = data['food_social']
                    self.tool_x, self.tool_y = data['tool']
        except Exception:
            pass
    ...
    def check_food_collision(self, body_x, body_y):
        # 1. Chequeo Comida Solo
        dist_solo = np.hypot(body_x - self.food_solo_x, body_y - self.food_solo_y)
        if dist_solo < 15.0:
            self.respawn_food('solo')
            return "SOLO"

        # 2. Chequeo Comida Social (Cualquier par de la tribu sirve)
        dist_social = np.hypot(body_x - self.food_social_x, body_y - self.food_social_y)
        if len(self.bodies) >= 2:
            # Contar cuántos están cerca de la comida social
            near_count = 0
            for b in self.bodies:
                d = np.hypot(b.x - self.food_social_x, b.y - self.food_social_y)
                if d < 30.0: near_count += 1
            
            if near_count >= 2:
                self.respawn_food('social')
                return "SOCIAL"
                    
        # 3. JACKPOT (Herramienta + Social): el cuerpo debe estar cerca también,
        # de lo contrario se otorgaba reward "fantasma" cuando la herramienta se
        # alineaba con la comida sin que ningún agente participara.
        tool_social_dist = np.hypot(self.tool_x - self.food_social_x, self.tool_y - self.food_social_y)
        body_tool_dist = np.hypot(body_x - self.tool_x, body_y - self.tool_y)
        if tool_social_dist < 20.0 and body_tool_dist < 30.0:
            self.respawn_food('social')
            self.respawn_tool()
            return "JACKPOT"
    ...
    def respawn_food(self, food_type):
        if food_type == 'solo':
            self.food_solo_x = np.random.uniform(50, 350)
            self.food_solo_y = np.random.uniform(50, 350)
        else:
...
class VirtualBody:
    """Cuerpo Orgánico (IA Viva 15.0): Gestiona energía, comunicación y órganos evolutivos."""
    def __init__(self, world: SharedWorld):
        ...
        self.speed = 0.0          
        self.reward  = 0.0
        self.metabolism_eff = 1.0 # Eficiencia base
        self.signal_output = np.zeros(8) # Voz de 8 canales
        
        self.dist_to_food_prev = np.inf
        self.jackpot_reward = False

    def get_sensory_input(self):
        """Retina Multi-Orgánica (14 Canales): visión, oído social e interoception
        computacional (estado emocional, RAM, CPU, buffer, tamaño de red)."""
        # Encontrar comida más cercana
        d_solo = np.hypot(self.x - self.world.food_solo_x, self.y - self.world.food_solo_y)
        d_social = np.hypot(self.x - self.world.food_social_x, self.y - self.world.food_social_y)

        if d_solo < d_social:
            target_x, target_y = self.world.food_solo_x, self.world.food_solo_y
            signature = 0.2
        else:
            target_x, target_y = self.world.food_social_x, self.world.food_social_y
            signature = 0.8
        ...
        # 1-5: Visión (Dist, Áng, Otro, Herramienta, Firma)
        s1 = 1.0 / (1.0 + dist / 100.0)
        s2 = rel_angle / np.pi

        # Oído Social: Detectar al compañero más cercano
        others = [b for b in self.world.bodies if b != self]
        if others:
            dists_others = [np.hypot(self.x - b.x, self.y - b.y) for b in others]
            closest_idx = np.argmin(dists_others)
            closest_other = others[closest_idx]
            s3 = 1.0 / (1.0 + dists_others[closest_idx] / 100.0)
            s6_9 = closest_other.signal_output[:4]
        else:
            s3 = 0.0
            s6_9 = [0.0]*4

        s4 = 1.0 / (1.0 + np.hypot(self.x - self.world.tool_x, self.y - self.world.tool_y) / 100.0)
        s5 = signature

        # 10-14: Interoception (estado interno emocional + carga del sistema)
        # El cerebro inyecta estos valores en _internal_state antes de cada step.
        state = getattr(self, '_internal_state', None)
        if state is not None:
            s10 = float(state.get('emotional', 0.5))  # frustración + falta de dopamina
            s11 = float(state.get('ram',       0.0))  # presión de memoria del sistema
            s12 = float(state.get('cpu',       0.0))  # presión de cómputo
            s13 = float(state.get('buffer',    0.0))  # saturación memoria episódica
            s14 = float(state.get('network',   0.0))  # tamaño propio (neuronas/300)
        else:
            s10 = s11 = s12 = s13 = s14 = 0.0

        return np.concatenate([[s1, s2, s3, s4, s5], s6_9, [s10, s11, s12, s13, s14]])

    def step(self, dt, motor_outputs: dict):
        """Gestiona el movimiento, señales y metabolismo."""
        self.reward = 0
        v_l = np.clip(motor_outputs.get('left', 0), 0, 1) * 5.0
        v_r = np.clip(motor_outputs.get('right', 0), 0, 1) * 5.0
        linear  = (v_l + v_r) * 0.5
        ...
        # El movimiento ya no cuesta energía (v17.0)
        self.metabolism_eff = 1.0 + motor_outputs.get('metabolism', 0) * 2.0

        # Señal de salida
        self.signal_output = np.array(motor_outputs.get('signals', [0]*8))

        # Física herramienta
        dist_tool = np.hypot(self.x - self.world.tool_x, self.y - self.world.tool_y)
        if dist_tool < 20.0:
            force = linear * 0.5
            self.world.tool_x += np.cos(self.angle) * force * dt
            self.world.tool_y += np.sin(self.angle) * force * dt
            self.world.tool_x = np.clip(self.world.tool_x, 10, self.world.WORLD_SIZE-10)
            self.world.tool_y = np.clip(self.world.tool_y, 10, self.world.WORLD_SIZE-10)

        # Colisión comida
        res = self.world.check_food_collision(self.x, self.y)
        if res:
            self.dist_to_food_prev = np.inf
            if res == "JACKPOT": self.reward = 15.0
            elif res == "SOCIAL": self.reward = 5.0
...
def _new_BrainSim_init(self, name="Unnamed", world=None):
    ...
    # 20.000 entradas para alinear con el cap explícito en _new_step
...
def _new_step(self):
    ...
    # ── Interoception computacional ──────────────────────────────────────────
    # El agente "siente" su propio estado emocional y la carga del sistema.
    # Ningún canal mata: el feedback es negativo (alivio = podar / parar).
    # Reescalado perezoso del buffer episódico para pkls viejos (lista plana
    # o deque con maxlen=200) → unifica a deque(maxlen=20000) recortando
    # historia muy antigua para dejar margen de crecimiento.
    buf = getattr(self, '_episodic_buffer', None)
    if buf is not None:
        needs_resize = (not isinstance(buf, deque)) or (getattr(buf, 'maxlen', None) != 20000)
        if needs_resize:
            try:
                # Si el buffer viene gigantesco (lista plana antigua), conservamos
                # solo los últimos 5000 para que el agente sienta que el buffer
                # se llena con la experiencia nueva, no con el cementerio histórico.
                items = list(buf)[-5000:]
                self._episodic_buffer = deque(items, maxlen=20000)
                buf = self._episodic_buffer
            except Exception:
                pass

    ram_pct, cpu_pct = _sample_system_resources(self.time)
    if buf is None or len(buf) == 0:
        buf_pct = 0.0
    else:
        cap = getattr(buf, 'maxlen', None) or 20000
        buf_pct = min(1.0, len(buf) / max(cap, 1))
    net_pct = min(1.0, len(neurons) / 300.0)
    emotional = float(np.clip(0.6 * self.frustration +
                              0.4 * (1.0 - self.neuromod.dopamine), 0.0, 1.0))
    self.body._internal_state = {
        'emotional': emotional,
        'ram':       float(ram_pct),
        'cpu':       float(cpu_pct),
        'buffer':    float(buf_pct),
        'network':   float(net_pct),
    }

    # Castigo por sobrecarga del SISTEMA (RAM/CPU). El buffer queda como señal
    # informativa: el agente lo siente pero el costo real es RAM/CPU.
    # Feedback negativo: si la red se reduce, el alivio llega solo.
    overload = 0.0
    if ram_pct > 0.80:
        overload += (ram_pct - 0.80) / 0.20
    if cpu_pct > 0.85:
        overload += (cpu_pct - 0.85) / 0.15
    if overload > 0:
        overload = min(1.0, overload)
        self.frustration = float(min(1.0, self.frustration + overload * 0.02))
        self.neuromod.dopamine = float(max(0.0, self.neuromod.dopamine - overload * 0.03))
    ...
    if self.brain_state == BrainState.AWAKE:
        retina = self.body.get_sensory_input() # 14 Canales (10 externos + 4 internos)
        sensory_layer = [n for n in neurons if n.pos[2] < 15.0]
        for i, sn in enumerate(sensory_layer):
            channel = i % len(retina)
            sn.sensory_current = retina[channel] * 800.0
            
    # ── Place Cells (15 < Z < 35) ... mantenido ...

    # ── Capas de Salida Evolved (Z > 35) ────────────────────────────────────
    motor_outputs = {'left': 0.0, 'right': 0.0, 'signals': [0.0]*8, 'metabolism': 0.0}
    
    # 35 < Z < 55: Motores
    m_layer = [n for n in neurons if 35.0 <= n.pos[2] < 55.0]
    if len(m_layer) >= 2:
        h = len(m_layer)//2
        motor_outputs['left'] = sum(n.firing_rate for n in m_layer[:h]) / (h*100.0 + 1e-6)
        motor_outputs['right'] = sum(n.firing_rate for n in m_layer[h:]) / (h*100.0 + 1e-6)
    
    # 55 < Z < 75: Emitters de Señal (Voz)
    e_layer = [n for n in neurons if 55.0 <= n.pos[2] < 75.0]
    for i, en in enumerate(e_layer):
        chan = i % 8
        motor_outputs['signals'][chan] = max(motor_outputs['signals'][chan], en.firing_rate / 100.0)
        
    # Z > 75: Órganos Metabólicos / Supervisor
    h_layer = [n for n in neurons if n.pos[2] >= 75.0]
    if h_layer:
        motor_outputs['metabolism'] = sum(n.firing_rate for n in h_layer) / (len(h_layer)*100.0 + 1e-6)
        
    # Ruido adaptativo de exploracion: alto cuando esta frustrado (busca nuevas
    # estrategias), bajo cuando esta comiendo bien (explota lo que funciona).
    # Equivalente computacional a la norepinefrina en biologia: el estres
    # aumenta la exploracion aleatoria.
    base_noise = 0.05
    if self.frustration > 0.5:
        base_noise = 0.15  # frustrado: explora mas
    elif self.body.reward > 0:
        base_noise = 0.01  # exito reciente: explota
    motor_outputs['left'] += np.random.uniform(0, base_noise)
    motor_outputs['right'] += np.random.uniform(0, base_noise)
    
    # Ejecutar paso físico con comandos orgánicos
    self.body.step(self.dt, motor_outputs)
    ...
    # ── Registrar distancia al alimento (IA Viva 15.0: más cercano) ────────────
    d_s1 = np.hypot(self.body.x - self.body.world.food_solo_x, self.body.y - self.body.world.food_solo_y)
    d_s2 = np.hypot(self.body.x - self.body.world.food_social_x, self.body.y - self.body.world.food_social_y)
    dist = min(d_s1, d_s2)
    ...
    if getattr(self, '_episodic_buffer', None) is None:
        self._episodic_buffer = []

    # Guardar momento si la dopamina es alta (experiencia exitosa).
    # El deque (maxlen=20000) descarta las entradas más antiguas automáticamente.
    ...
    if self.body.reward > 0:
        ...
        for n in neurons:
            if self.time - n.last_spike_time < 40.0:
                for syn in n.synapses_in:
                    syn.myelination = min(1.0, syn.myelination + 0.1)
                    syn.weight = min(1.5, syn.weight * (1.0 + syn.myelination * 0.05))

    # IA Viva 7.0 - Feature 3: Neuro-Evolucion (Seleccion Natural) ─────────
    # Sistema simetrico: crecimiento durante alta dopamina + decay metabolico
    # SIEMPRE activo. Antes el decay solo corria durante baja dopamina, lo que
    # permitia inflacion de utilidad durante feasts prolongados (utilidades
    # observadas hasta 108.0). Ahora la presion metabolica regula incluso
    # durante alta dopamina: el costo energetico de mantener neuronas activas
    # es real y constante.

    # 1) Crecimiento (solo durante alta dopamina, premia disparos asociados a recompensa)
    if self.neuromod.dopamine > 0.65:
        for n in neurons:
            if self.time - n.last_spike_time < 80.0:  # Disparo reciente
                self._neuron_utility[n.id] = self._neuron_utility.get(n.id, 0.5) + 0.01

    # 2) Decay metabolico (siempre activo, escala con utilidad total)
    # Si la utilidad total excede el budget (n_neuronas * 1.5), el decay se acelera
    # para todas las neuronas: competencia real por ATP, sin tope artificial.
    n_neurons_count = max(1, len(neurons))
    atp_budget = n_neurons_count * 1.5
    total_utility = sum(self._neuron_utility.get(n.id, 0.5) for n in neurons)
    metabolic_pressure = max(1.0, total_utility / atp_budget)
    decay_rate = 0.9995 ** metabolic_pressure
    for n in neurons:
        if n.id in self._neuron_utility:
            self._neuron_utility[n.id] = max(0.0, self._neuron_utility[n.id] * decay_rate)

    # Reconciliación periódica de utilidad: descarta IDs de neuronas que ya no
    # existen (limpia el cementerio histórico acumulado en pkls antiguos para
    # que la selección darwiniana opere sobre la red real, no sobre fantasmas).
    if self.step_count % 1000 == 0 and isinstance(self._neuron_utility, dict) and self._neuron_utility:
        live_ids = {c.id for c in self.cells}
        stale = [k for k in self._neuron_utility if k not in live_ids]
        for k in stale:
            del self._neuron_utility[k]

    # Musk Step 2: Extreme Apoptosis si la red está en "Parálisis Zen" (Idle)
    mass_extinction = self._idle_steps > 1500
    # IA Viva 13.1: Inmunidad Social (No castigar la espera en Era 2+)
    if self.body.world.era >= 2 and dist < 40.0:
        mass_extinction = False
        
    # v17.0: Parsimonia y Sparsity (Selección por Utilidad)
    # Musk poda basado en la arquitectura, no en el hambre
    if self.step_count % 500 == 0 or mass_extinction:
        to_kill = []
        for n in neurons:
            utility = self._neuron_utility.get(n.id, 0.5)
            # Eliminar neuronas aisladas inmediatamente (Código Muerto)
            if len(n.synapses_in) == 0 and len(n.synapses_out) == 0 and len(neurons) > 10:
                to_kill.append(n.id)
                continue
        ...
        # v17.0: El Brote nace de la Frustración o el Vacío
        ...
        if systemic_panic or len(neurons) < 4:
            ...
            print(f"[+] IA VIVA 17.0: BROTE POR FRUSTRACIÓN. {mutations} nuevas neuronas.", flush=True)
            for _ in range(mutations):
                ...
                c.synapses_in = []
                c.synapses_out = []
                self._neuron_utility[c.id] = 1.0 # Inmunidad inicial
                self.cells.append(c)
    # ── Guardar historia ──────────────────────────────────────────────────────
    if self.history:
...
def _incremental_structural_plasticity(sim, neurons):
    ...
    # Nos quedamos con las que sobreviven y luego limpiamos las referencias
    # huérfanas en synapses_out de las pre-sinápticas (antes generaban fuga).
    surviving = set()
    for n in neurons:
        keep = [s for s in n.synapses_in if s.weight > 0.01 * da_mod]
        n.synapses_in = keep
        surviving.update(id(s) for s in keep)
    for n in neurons:
        n.synapses_out = [s for s in n.synapses_out if id(s) in surviving]
...
class BrainVisualizer:
    def __init__(self, sim):
        ...
        try:
            self.fig.canvas.manager.set_window_title('IA Viva 15.1 - Tribu Evolutiva')
        except Exception:
    ...
    def update(self, frame):
        ...
        # Comida más cercana entre solo y social (los atributos viven en SharedWorld)
        d_solo = np.hypot(body.x - body.world.food_solo_x, body.y - body.world.food_solo_y)
        d_social = np.hypot(body.x - body.world.food_social_x, body.y - body.world.food_social_y)
        if d_solo <= d_social:
            food_x, food_y = body.world.food_solo_x, body.world.food_solo_y
        else:
            food_x, food_y = body.world.food_social_x, body.world.food_social_y
        self.food_dot.set_data([food_x], [food_y])
        self.food_aura.set_data([food_x], [food_y])
        ...
        dist_now = min(d_solo, d_social)
        n_syn = sum(len(n.synapses_in) for n in neurons)
        self.fig.suptitle(
            f"IA Viva 15.1 (Tribu Evolutiva)  ·  {state_str}  ·  t={current_time:.0f} ms  ·  "
...
if __name__ == "__main__":
    import time
    print("=" * 80)
    print("IA VIVA 15.1 - EL CLAN DE LOS CUATRO (Tribu Evolutiva)")
    print("ALFA | BETA | GAMMA (Ancestral) | DELTA (Ancestral)")
    print("=" * 80)
    
    # 1. Preparar el Mundo con Persistencia
    mundo = SharedWorld()
    mundo.load_world()
    
    # 2. Cargar a los 4 Combatientes
    sim_alfa = BrainSimulation(name="Alfa", world=mundo)
    sim_beta = BrainSimulation(name="Beta", world=mundo)
    sim_gamma = BrainSimulation(name="Gamma", world=mundo)
    sim_delta = BrainSimulation(name="Delta", world=mundo)
    
    print("\n[CARGANDO CEREBROS VETERANOS Y ANCESTRALES...]")
    # Rutas ancestrales
    path_anc_alfa = "IA_Viva_Arena_Darwiniana_V2/IA_Viva_Arena_Darwiniana/arena_save_alfa.pkl"
    path_anc_beta = "IA_Viva_Arena_Darwiniana_V2/IA_Viva_Arena_Darwiniana/arena_save_beta.pkl"
    
    try:
        # Veteranos (Laptop/Arena actual)
        sim_alfa.load_brain("arena_save_alfa.pkl")
        sim_beta.load_brain("arena_save_beta.pkl")
        
        # Ancestros (Escritorio - Destilados)
        sim_gamma.load_brain(path_anc_alfa)
        sim_delta.load_brain(path_anc_beta)
        print("[!] TRIBU COMPLETA: 2 Veteranos + 2 Ancestros cargados.")
    except Exception as e:
        print(f"Error cargando la tribu: {e}")
        
    print(f"\n[INICIANDO ECOSISTEMA DE TRIBU EN {mundo.WORLD_SIZE}um x {mundo.WORLD_SIZE}um]")
    print("Presiona CTRL+C para detener y guardar todo.\n")
    ...
    try:
        while True:
            ...
            # Telemetría 15.1 (Tribu)
            def get_aura(signals):
                return "".join(['H' if s > 0.5 else '-' for s in signals[:4]])
            
            def get_stat(sim):
                n = len([c for c in sim.cells if c.type == CellType.NEURON])
                return f"N:{n} FR:{sim.frustration:.1f} {get_aura(sim.body.signal_output)}"

            print(f"[{mundo.step_count/1000:,.1f}K] A({get_stat(sim_alfa)}) | B({get_stat(sim_beta)}) | G({get_stat(sim_gamma)}) | D({get_stat(sim_delta)}) | FPS:{fps:.0f}")

            # Auto-guardado de seguridad
            if mundo.step_count % 100000 == 0:
                mundo.save_world()
                sim_alfa.save_brain("arena_save_alfa.pkl")
                sim_beta.save_brain("arena_save_beta.pkl")
                sim_gamma.save_brain("arena_save_gamma.pkl")
                sim_delta.save_brain("arena_save_delta.pkl")
                # Actualizar bitácora en segundo plano (no bloquea la simulación)
                try:
                    import subprocess as _sp, sys as _sys
                    _sp.Popen([_sys.executable, "bitacora.py", "--silent"],
                              cwd=os.path.dirname(os.path.abspath(__file__)))
                except Exception:
                    pass

    except KeyboardInterrupt:
        print("\n[!] TRIBU DETENIDA POR EL USUARIO.")
        mundo.save_world()
        sim_alfa.save_brain("arena_save_alfa.pkl")
        sim_beta.save_brain("arena_save_beta.pkl")
        sim_gamma.save_brain("arena_save_gamma.pkl")
        sim_delta.save_brain("arena_save_delta.pkl")
        print("Estado de la Tribu guardado exitosamente.")
        # Actualizar bitácora al detener manualmente
        try:
            import subprocess as _sp, sys as _sys
            _sp.run([_sys.executable, "bitacora.py"],
                    cwd=os.path.dirname(os.path.abspath(__file__)))
        except Exception:
            pass
