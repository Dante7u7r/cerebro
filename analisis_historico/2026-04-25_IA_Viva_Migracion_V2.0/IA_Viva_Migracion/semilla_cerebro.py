...
3. Plasticidad a Corto Plazo STP (Facilitacion y Depresion sinaptica)
4. Astrocitos activos (modulacion sinaptica, reciclaje de glutamato)
5. Mielinizacion y velocidad de conduccion variable
6. Gradientes morfogenos y formacion de capas corticales
7. Oscilaciones de red (LFP y espectrograma)
8. Homeostasis sinaptica (scaling)
9. Ciclo sueño-vigilia y consolidacion de memoria
10. Neuromodulacion (Dopamina, Serotonina, Acetilcolina)
11. Computacion dendrtica (compartimentos soma/dendrita)
12. Microgla y poda sinaptica
13. Acoplamiento neurovascular (energa local)
...
# --- Optimizacion vectorial (AMD/CPU) ---
def _cdist_np(A, B):
    """Distancia euclidea vectorizada (equivalente a scipy.spatial.distance.cdist)."""
...
DT = 0.1                     # Paso de integracion (ms)
SIGNAL_PERIOD = 127.7        # 7.83 Hz (Resonancia de Schumann)
SLEEP_CYCLE_PERIOD = 2000.0  # Periodo del ciclo sueño-vigilia (ms)
...
class NeuronSubtype(Enum):
    EXCITATORY = 1      # Glutamatergica (piramidal)
    INHIBITORY_PV = 2   # Parvalbumina: inhibicion rapida al soma
    INHIBITORY_SST = 3  # Somatostatina: inhibicion lenta a dendritas
...
# ============================================================================
# RED GENETICA (Hes1/Ngn2/Mash1/Dlx)
# ============================================================================
...
class GeneRegulatoryNetwork:
    ...
    def update(self, dt, external_bias, morphogen_signal):
        ...
        # Dlx: inducido por Mash1, influenciado por morfogeno (mas en profundidad)
...
class BiologicalSynapse:
    def __init__(self, pre_id, post_id, pre_subtype, delay=0.0, target_compartment='soma'):
        ...
        self.u_stp = self.U_stp   # Probabilidad de liberacion actual
        self.x_stp = 1.0          # Recursos vesiculares disponibles (0-1)
        self.tau_d = 200.0        # Recuperacion de depresion (ms)
        self.tau_f = 50.0         # Decaimiento de facilitacion (ms)
    ...
    def pre_spike(self, time):
        ...
        # Dinamica STP: facilitacion y depresion
...
class Astrocyte:
    ...
    def update(self, dt, neurons, synapses, local_energy=1.0):
        # Actividad glutamatergica local
        local_activity = 0.0
        for n in neurons:
        ...
        # Acoplamiento neurovascular: baja energia → vasodilatacion
        if local_energy < 0.5:
...
class Oligodendrocyte:
    ...
    def myelinate(self, synapse, distance):
        # A mayor maduracion y cercania, mas mielinizacion
...
class NeuromodulatorSystem:
    """Sistema de neuromodulacion global que modula plasticidad y excitabilidad."""
    def __init__(self):
        self.dopamine = 0.5       # Señal de novedad/recompensa
        self.serotonin = 0.5      # Regulacion del animo y estado
        self.acetylcholine = 0.5  # Atencion y excitabilidad
        self.tau_da = 200.0   # ms
        self.tau_5ht = 500.0  # ms
        self.tau_ach = 100.0  # ms
        
    def update(self, dt, brain_state, prediction_error):
        # Dopamina: impulsada por error de prediccion (novedad)
        ...
        # Serotonina: tonica en vigilia, baja en REM
        ...
        # Acetilcolina: alta en vigilia activa, baja en sueño lento
        if brain_state == BrainState.AWAKE:
            ach_target = 0.8
        elif brain_state == BrainState.SLOW_WAVE_SLEEP:
            ach_target = 0.2
    ...
    def stdp_modulation(self):
        """Dopamina modula la ventana STDP: alta DA = mas plasticidad."""
        return 0.5 + self.dopamine  # Rango: 0.5 a 1.5
        
    def excitability_modulation(self):
        """ACh modula excitabilidad neural: alta ACh = umbral mas bajo."""
        return -3.0 * self.acetylcholine  # Baja umbral hasta -3 mV

# ============================================================================
# MICROGLIA (Poda Sináptica)
# ============================================================================

class Microglia:
    """Célula inmune del SNC que patrulla y poda sinapsis débiles/inactivas."""
    def __init__(self, cell_id, position):
        self.id = cell_id
        self.pos = position.copy()
        self.radius = 45.0
        self.patrol_speed = 0.5  # μm/ms
        self.pruning_threshold = 0.02  # Peso minimo para conservar sinapsis
        self.activity_threshold = 500.0  # ms de inactividad maxima
    ...
    def prune(self, neurons, current_time):
        """Elimina sinapsis débiles o inactivas cerca de esta microglia."""
        pruned = 0
...
class IntelligentCell:
    ...
    def update_biology(self, dt, neighbor_delta, ext_signal, neurotrophic, morphogen, brain_state):
        ...
        # Energia base (ajustada para que no mueran de inanición rápido)
        self.energy -= 0.1 * dt
        if self.state == CellState.PROLIFERATIVE: self.energy -= 2.0 * dt
        if self.state == CellState.QUIESCENT: self.energy += 0.3 * dt
        # Regeneración basal de energía (perfusión sanguínea)
        ...
        if self.type == CellType.STEM:
            ...
            if lt == CellType.NEURON and diff_score > 1.0:
                ...
                # Subtipo según Dlx y morfógeno: PV+ o SST+
                if self.grn.inhibitory_bias() > 0.4 or morphogen > 0.5:
                    if self.grn.D_protein > 0.6 or np.random.rand() < 0.6:
                        ...
                        self.tau_m = 10.0    # PV+ es rápida
                        self.v_thresh = -45.0
                    else:
                        self.subtype = NeuronSubtype.INHIBITORY_SST
                        self.tau_m = 25.0    # SST+ es más lenta
                        self.v_thresh = -52.0
                else:
        ...
        # Migración guiada por gradiente
    ...
    def _refractory_period(self):
        """Periodo refractario según subtipo neuronal."""
        if self.subtype == NeuronSubtype.INHIBITORY_PV:
            return 2.0   # PV+ dispara rápido
        elif self.subtype == NeuronSubtype.INHIBITORY_SST:
            return 5.0
        else:
            return 5.0   # Piramidal estándar
    ...
    def update_neural(self, dt, time, input_current, brain_state, neuromod=None):
        ...
        # Modulación de excitabilidad por neuromoduladores y energía (neurovascular)
        thresh_mod = 0.0
        if neuromod is not None:
            thresh_mod += neuromod.excitability_modulation()
        # Baja energía sube el umbral (más difícil disparar)
        ...
        # Spike dendrítico: inyección de corriente fuerte al soma
        ...
        # Homeostasis sináptica (cada 1000 ms)
    ...
    def update_glia(self, dt, neurons, synapses, local_energy=1.0):
        ...
        if self.subtype == GliaSubtype.ASTROCYTE:
            ...
            # Neurovascular: astrocito boost energía de neuronas cercanas
...
class BrainSimulation:
    def __init__(self):
        ...
        # Células iniciales (v5.0: Arquitectura por Capas)
    ...
    def compute_synaptic_connections(self):
        """Reconfigura conexiones según proximidad y mielinización."""
        ...
        for pre in neurons:
            ...
            for post in neurons:
                ...
                # Radio de conexión según subtipo
                ...
                if dist < max_dist and np.random.rand() < p_connect:
                    ...
                    # Compartimento destino: PV+→soma, SST+→dendrita, Exc→soma
    ...
    def compute_lfp(self):
        ...
        for cell in self.cells:
            ...
            if dist < 100.0:
                # Contribución proporcional a corriente sináptica total
    ...
    def step(self):
        ...
        # Señal objetivo (senoidal + ruido) - solo en vigilia
        if self.brain_state == BrainState.AWAKE:
            target = np.sin(2 * np.pi * self.time / SIGNAL_PERIOD) + 0.1 * np.random.randn()
        else:
            target = 0.0  # Sin entrada durante sueño
        self.target_signal_history.append(target)
        
        # Reconfigurar sinapsis periódicamente (ELIMINADO: causaba pérdida de memoria)
        # if self.step_count % 100 == 0:
        #     self.compute_synaptic_connections()
            
        # Obtener neuronas y glía para actualizaciones
        neurons = [c for c in self.cells if c.type == CellType.NEURON]
        ...
        # Actualizar microglía y poda sináptica (cada 500 pasos)
        for mg in microglia:
        ...
        # Calcular neighbor_deltas y morfógenos en bloque (vectorizado)
        ...
        # Dinámica neuronal
        # Decaer conductancias sinápticas
        for cell in neurons:
            for syn in cell.synapses_in:
                syn.update_conductances(self.dt)
        # Actualizar entradas sinápticas
        for cell in neurons:
            input_current = 0.0
            # En vigilia, añadir corriente de entrada sensorial
            if self.brain_state == BrainState.AWAKE:
        ...
        # Después del disparo, aplicar STDP modulado por dopamina
...
class VirtualBody:
    ...
    def step(self, dt, motor_left: float, motor_right: float):
        ...
        # Diferencia de velocidades determina giro (modelo de tanque)
        ...
        self.speed  = linear
        # Movimiento tipo Pac-Man (si sale por un lado, entra por el otro) para no quedarse atascado
        self.x = (self.x + np.cos(self.angle) * linear * dt) % self.WORLD_SIZE
        self.y = (self.y + np.sin(self.angle) * linear * dt) % self.WORLD_SIZE

        dist = np.hypot(self.x - self.food_x, self.y - self.food_y)
        # Recompensa: mejora en distancia
        if self.dist_to_food_prev != np.inf:
            self.reward = np.clip((self.dist_to_food_prev - dist) * 0.1, -1, 1)
        self.dist_to_food_prev = dist

        if dist < 15.0:
            self.steps_on_food += 1
            if self.steps_on_food > 20:   # llego a la comida -> nueva posicion
                self.food_x = np.random.uniform(50, 350)
                self.food_y = np.random.uniform(50, 350)
                self.steps_on_food = 0
                # Resetear distancia previa para evitar recompensa espuria al teletransportar
                self.dist_to_food_prev = np.inf
        else:
            self.steps_on_food = 0

    def get_sensory_input(self):
        """Retina de 5 canales: [Lejos_Izquierda, Cerca_Izquierda, Centro, Cerca_Derecha, Lejos_Derecha]"""
        dx = self.food_x - self.x
        dy = self.food_y - self.y
        dist = np.hypot(dx, dy)
        angle_to_food = np.arctan2(dy, dx)
        rel_angle = (angle_to_food - self.angle + np.pi) % (2 * np.pi) - np.pi
        
        retina = np.zeros(5)
        angles = np.array([-0.8, -0.4, 0.0, 0.4, 0.8]) # abanico de sensores
        for i, a in enumerate(angles):
            # La respuesta del sensor es maxima si el angulo coincide y estamos cerca
...
def _new_step(self):
    ...
    dist = np.hypot(self.body.x - self.body.food_x, self.body.y - self.body.food_y)
    ...
    grid_x = int(min(19, self.body.x / VirtualBody.WORLD_SIZE * 20))
    grid_y = int(min(19, self.body.y / VirtualBody.WORLD_SIZE * 20))
    ...
    if self.step_count % 500 == 0 or mass_extinction:
        ...
        if to_kill:
            ...
            if mass_extinction:
                ...
                self.body.x = np.clip(self.body.x, 0, VirtualBody.WORLD_SIZE)
                self.body.y = np.clip(self.body.y, 0, VirtualBody.WORLD_SIZE)
                self._idle_steps = 0
    ...
    # IA Viva 12.0: Auto-Guardado de Seguridad (Cada 100,000 pasos)
    if self.step_count % 100000 == 0:
        try:
            self.save_brain("brain_save.pkl")
            print("[AUTO-SAVE] Progreso evolutivo respaldado para evitar perdidas.", flush=True)
        except Exception:
            pass
...
def save_brain(sim, path="brain_save.pkl"):
...
if __name__ == "__main__":
    import time
    print("=" * 70)
    print("IA VIVA 7.0 - CEREBRO EMERGENTE  |  Modo: NÚCLEO PURO (Sin Interfaz)")
    print("Velocidad de simulación: ILIMITADA")
    print("=" * 70)
    
    sim = BrainSimulation()
    try:
        sim.load_brain()  # Intentar cargar guardado previo
    except Exception as e:
        print("Iniciando cerebro desde cero.")
    
    print("\n[INICIANDO ENTRENAMIENTO INTENSIVO. PRESIONA CTRL+C PARA DETENER Y GUARDAR]\n")
    ...
    try:
        while True:
            # Correr un lote de pasos lo más rápido posible
            ...
            neurons = [c for c in sim.cells if c.type == CellType.NEURON]
            n_syn = sum(len(n.synapses_in) for n in neurons)
            dist_now = np.hypot(sim.body.x - sim.body.food_x, sim.body.y - sim.body.food_y)
            da = sim.neuromod.dopamine
            
            # Formato ultra-compacto y seguro para terminales Windows
            s_bio = f"{sim.time/1000:.1f}s"
            s_red = f"N:{len(neurons)} S:{n_syn}"
            s_emc = f"F:{sim.frustration:.1f} R:{sim.resilience:.1f} V:{sim.courage:.1f}"
            print(f"[{steps/1000:,.0f}K] {s_bio} | {s_red} | D:{dist_now:.0f} DA:{da:.2f} | {s_emc} | {fps:.0f} FPS")
                  
    except KeyboardInterrupt:
        print("\n[!] ENTRENAMIENTO DETENIDO POR EL USUARIO.")
        print("Guardando estado del cerebro...")
        sim.save_brain()
